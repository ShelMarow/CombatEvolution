...
public class BehaviorUtils {
    ...
    public static CEAnimationAttackGoal<?> getCEAnimationAttackGoal(LivingEntity livingEntity){
        if (livingEntity instanceof Mob mob){
            ...
            if (goal.isPresent()){
                return (CEAnimationAttackGoal<?>) goal.get().getGoal();
            }
        }
        return null;
    }
    ...
    public static void changeCurrentBehavior(MobPatch<?> entityPatch, String rootName, String behaviorName, boolean ignoreCondition) {
        stopCurrentBehavior(entityPatch.getOriginal());
        CECombatBehaviors<?> combatBehaviors = getCECombatBehaviors(entityPatch);
        if (combatBehaviors != null) {
            combatBehaviors.changeBehavior(entityPatch, rootName, behaviorName, ignoreCondition);
        }
    }
